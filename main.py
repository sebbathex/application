...
#    conn = sqlite3.connect('corneal_dystrophies_database.db')
...
def submitted():
   if request.method == 'GET':
        ...
        #lade bilder un beschriebungen
        
        column = erster
        query = f"SELECT short_explanation FROM info_table_corneal_dystrophies WHERE name = {column}"
        query1 = f"SELECT image_name FROM info_table_corneal_dystrophies WHERE name = {column}"
        

        conn = sqlite3.connect('corneal_dystrophies_database.db')
        post = conn.execute(query).fetchone()
        imagename = conn.execute(query1).fetchone()
        print(post)
        print('Hallo' + f'{imagename}')
        conn.close()
        bild = os.path.join(app.config['UPLOAD_FOLDER'], f'{imagename[0]}')
        ...
        return render_template('ergebnis.html', decision_tree=tree.replace("'", "") , random_forest =forest.replace("'", ""), k_neigh=neighbors.replace("'", ""), log_reg= regression.replace("'", ""), 
            bayes= bayesian.replace("'", ""), perceptron= perception.replace("'", ""), supportvector= support.replace("'", ""), erster_platz = first_place, zweiter_platz= second_place, picture = bild, text = post[0].replace("'", ""))
